...
到目前为止，我们在 `index.html` 文件中手动引入所有资源，然而随着应用程序规模增长，当你开始输出[文件名哈希(hash)](/guides/caching)]并输出[多个 bundle](/guides/code-splitting)时，仍然采用手动方式对 `index.html` 文件进行管理将会非常困难。不过，有一些插件会使这个过程更加容易。
